...
}  else
{
  if (finalLinkas === undefined) {
     for (let w = 0; w < responseFinalBase.offerwallItems.length; w++) {
      referalLink.push("/sec/" + responseFinalBase.offerwallItems[w].offer.secondLander.link +"?")
     }

  }
  else {
   for (let u = 0; u < responseFinalBase.offerwallItems.length; u++) {
    referalLink.push("/sec/"+ responseFinalBase.offerwallItems[u].offer.secondLander.link +"?"+finalLinkas)
   }
  }
